  ...
  public String send(String request, Arguments args, Status status, String action) throws IOException {
    ...
    while (retry) {
      try {
        ...
        log.debug("The " + action + "-request execution time: " + execTime + " ms. (serialnumber: " + serialNumber + ")");
        retry = false;
    ...
    if (statusCode != HttpStatus.SC_OK) {
      if (statusCode != HttpStatus.SC_NO_CONTENT)
        log.error("The HTTP response from the server is NOT ok: " + statusCode + " (serialnumber: " + serialNumber + ")");
      return "";
    }
    ...
    log.info("Time not spent on send/receive in send() " + ((endSend - startSend) - execTime) + " ms. (serialnumber: " + serialNumber + ")");
    return response;
  }
